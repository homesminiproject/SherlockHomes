							...
							<div class="row" style="padding-left: 20px">
								<div class="card card-margin search-form">
									<div class="card-body p-0">
										<form action="SearchUser.do8" method="post">
											<div class="row">
												<div class="col-12">
													<div class="row no-gutters">
														<div class="col-lg-3 col-md-3 col-sm-12 p-0">
															<label for="search-type" hidden>검색 유형</label> <select
																class="form-control" id="searchCondition"
...
%>
							<div class="row-lg-12" id="boardBox">
								<table class="table" id="article-table">
									...
									<!-- 내용 -->
									<tbody>
										<% for (UsersDTO users :usersList) { %>
											<tr>
												<td><%= users.getNo()%></td>
												<td><%= users.getId()%></td>
												<td><a href="getUser.do8?no=<%= users.getNo()%>"><%= users.getName()%></a></td>
												<td><%= users.getRegion()%></td>
												<td><%= users.getRegdate()%></td>
												<td><%= users.getBirth()%></td>
												<td><%= users.getRole()%></td>
												<td><a href="getUser.do8?no=<%= users.getNo()%>"><i
														class="mdi mdi-pencil menu-icon"></i></a> | <a href="deleteUser.do8?no=<%= users.getNo()%>"><i
        ...
        <nav id="pagination" aria-label="Page navigation">
            <ul class="pagination justify-content-center">
                <li class="page-item">
                    <a class="page-link" href="listUser.do8?page=<%= prevPage %>" aria-label="Previous">
                        <span aria-hidden="true">&laquo;</span>
                    </a>
                </li>
                <% for (int i = 1; i <= totalPages; i++) { %>
                    <li class="page-item <%= i == currentPage ? "active" : "" %>">
                        <a class="page-link" href="listUser.do8?page=<%= i %>"><%= i %></a>
                    </li>
                <% } %>
                <li class="page-item">
                    <a class="page-link" href="listUser.do8?page=<%= nextPage %>" aria-label="Next">
