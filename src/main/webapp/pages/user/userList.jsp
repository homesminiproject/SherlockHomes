						...
						<div class="card">
							...
							<div class="row-lg-12" id="boardBox">
								<table class="table" id="article-table">
									...
									<!-- 내용 -->
									<tbody>
										<c:forEach begin="${startUserIndex}" end="${endUserIndex}"
											...
											<tr>
												...
												<td><a href="getUser.do8?no=${users.getNo()}"><i
														class="mdi mdi-pencil menu-icon"></i></a> | <a href="#"
													class="delete-user" data-no="${users.getNo()}"><i
							...
							<div style="text-align: center; margin-top: 10px;">
								<c:if test="${hasPreviousPage}">
									<a href="listUser.do8?page=${currentPage - 1}"
										style="font-size: 14px;">이전</a>
								</c:if>
								<c:forEach begin="0" end="${total/10}" var="i">
									<a href="listUser.do8?page=${i+1}"
										style="font-size: 14px; margin-left: 10px;">${i+1}</a>
								</c:forEach>
								<c:if test="${hasNextPage}">
									<a href="listUser.do8?page=${currentPage + 1}"
										style="font-size: 14px; margin-left: 10px;">다음</a>
								</c:if>
							</div>
